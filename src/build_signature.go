...
func NewRef(fidx int, filename string) *Ref {
    return &Ref{
        fidx:   fidx,
        filename: filename, 
    }
...
//-----------------------------------------------------------------------------
func ScanRefs2Channel(refseq string) chan Ref {
    // Walk through refseq dir 
    fscanner := ppt_filter.NewFileScanner(refseq)
    defer utils.TimeConsume(time.Now(), "Run time - ScanRefs2Channel: ")

    refs_channel := make(chan Ref, runtime.NumCPU())
    go func() {
        for fidx, filename := range fscanner.Scan() {
            fmt.Println("ScanRefs2Channel", fidx, filename)
            refs_channel <- (*NewRef(fidx, filename))
        }
...
func VerifySignature(f *ppt_filter.Filter, refseq string, k int, ph int) {
    ...
    var wg2_add_kmers sync.WaitGroup
    ...
    for ref := range refs_channel {
        ...
        go func(fidx int, filename string) {
                ...
                for fa_scanner.Scan() {
                    ...
                    // f.Gid[uint16(ref.fidx+1)] = fa_scanner.Header[1:]
