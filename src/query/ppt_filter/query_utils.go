...
func PrintOnlineResult(f *Filter, idx uint16, read_1 []byte, read_2 []byte, kmer []byte, bacteria_map map[uint16]*Bacteria, header_1 string, header_2 string, genome_info map[string]string, level string) {
	...
	if genome_name != "Not found." {
		true_fasta := fasta_folder + genome_name + ".fa"
	...
	predicted_fasta := fasta_folder + f.Gid[idx] + ".fa"
...
func LoadGenomeInfo(level string) map[string]string{
	file := "/media/admin1/0367988c-be76-43c0-a243-48aafc97ffd4/dpham2/mende_metagenomics_data/scripts/new_groupRef2_genome_reference.csv"
	
	if level == "species" {
		file = "/media/admin1/0367988c-be76-43c0-a243-48aafc97ffd4/dpham2/mende_metagenomics_data/scripts/gID_species.csv"
	}
