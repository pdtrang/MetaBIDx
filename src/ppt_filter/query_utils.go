...
func PrintOnlineResult(f * Filter, idx uint16, read_1 []byte, read_2 []byte, kmer []byte, bacteria_map map[uint16]*Bacteria) {
	...
	if strings.Contains(string(read_1), string(kmer)) || strings.Contains(string(RevComp(read_1)), string(kmer)) {
		fmt.Println("Kmer is in Read 1")
		fmt.Println("Read 1: ", string(read_1))
	} else if strings.Contains(string(read_2), string(kmer)) || strings.Contains(string(RevComp(read_2)), string(kmer)) {
